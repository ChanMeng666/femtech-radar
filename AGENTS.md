...
- **Live site:** https://chanmeng666.github.io/femtech-radar/ · **RSS:** https://chanmeng666.github.io/femtech-radar/rss.xml
...
Deferred to a future version (NOT yet built): the `opportunities` and `discussions` source adapters,
ChatOps slash commands, and per-section RSS feeds.
...
Each Monday it launches the published MCP via `npx`, calls `radar_collect` for `industry` + `research`,
and the Copilot agent curates the results: picks top items, writes a one-sentence `why_it_matters` per
item, writes the weekly `editor_note`, then emits two `safe-outputs`:
...
- `/` (latest week, 4 sections, pinned `editor_note`) · `/archive` · `/week/[week]` · `/sources` · `/rss.xml`.
...
Unit ② additionally writes a `why_it_matters` string on each item (the editorial payload). In v1 only
`industry` + `research` have content; `opportunities`/`discussions` are empty arrays.
...
- **Graceful degradation:** a failing source yields `[]` + a warning from `collect()`; it never throws out. Adapters propagate errors; `collect()` converts them to warnings.
